...
format_image() {
    ...
    if [ ! -z $ZERO_LENGTH ]; then
        ...
        sudo dd if=/dev/zero of=${TARGET} bs=1M count=${ZERO_LENGTH}
    fi
    # Label the drive
    sudo parted -s ${TARGET} mklabel msdos
    if [ ! -z $BOOT_SIZE ]; then
        echo "Creating a boot partition"
        sudo parted -s "${TARGET}" unit MB mkpart primary fat32 $BOOT_OFFSET $((BOOT_SIZE+BOOT_OFFSET))
    ...
    sudo parted -s "${TARGET}" unit "${ROOT_UNIT}" mkpart primary $ROOT_OFFSET 100%
    if [ ! -z "$ROOT_FORMAT_OPTIONS" ]; then
        echo "Formatting root partition with options"
        sudo mkfs.ext4 $ROOT_FORMAT_OPTIONS "${ROOT_PART}"
    else
...
mount_image() {
    mkdir -p "${ROOT_PATH}"
    sudo mount "${ROOT_PART}" "${ROOT_PATH}"
    if [ ! -z "${BOOT_PART}" ]; then
...
banana_pi() {
    ...
    wget -c -N https://raw.githubusercontent.com/bowseruk/ArchLinuxArm-Image-Create/image/BANANA_PRO/boot/boot.cmd -O "${WORKING_PATH}/${BOARD}/boot/boot.cmd"
    ...
    if [ ! -f "${WORKING_PATH}/${BOARD}/dd/u-boot-sunxi-with-spl.bin" ]; then
        ...
        if [ ${BUILD_ARCH} = "armv7l" ]; then
            sudo make -j4 "${DEFCONFIG}"
            sudo make -j4
        else
            sudo make -j4 ARCH=arm CROSS_COMPILE=arm-none-eabi- "${DEFCONFIG}"
            sudo make -j4 ARCH=arm CROSS_COMPILE=arm-none-eabi-
        fi
        cp "/tmp/git/${BOARD}/u-boot/u-boot-sunxi-with-spl.bin" "${WORKING_PATH}/${BOARD}/dd/u-boot-sunxi-with-spl.bin"
        sudo rm -r "/tmp/git/${BOARD}/u-boot"
        cd "${WORKING_PATH}"
    fi
}
...
install_root() {
    ...
    sudo bsdtar -xpf "${WORKING_PATH}/Includes/${IMAGE}" -C "${ROOT_PATH}"
    if [ -d "${WORKING_PATH}/${BOARD}/boot" ]; then
        sudo cp -r "${WORKING_PATH}/${BOARD}/boot/." "${BOOT_PATH}/"
    fi
}
...
select_board() {
    case $BOARD in
        [Bb][Aa][Nn][Aa][Nn][Aa]_[Pp][Ii] | [Bb]_[Pp][Ii] )
            ...
            install_root
            umount_image
            sudo dd if="${WORKING_PATH}/${BOARD}/dd/u-boot-sunxi-with-spl.bin" of="${TARGET}" bs=1024 seek=8
            ;;
        ...
        [Rr][Oo][Cc][Kk]64 | [Rr]64 )
            ...
            install_root
            umount_image
            sudo dd if="${WORKING_PATH}/${BOARD}/dd/rksd_loader.img" of=$TARGET seek=64 conv=notrunc
            sudo dd if="${WORKING_PATH}/${BOARD}/dd/u-boot.itb" of=${TARGET} seek=16384 conv=notrunc
            ;;
        *)
            usage
            exit
